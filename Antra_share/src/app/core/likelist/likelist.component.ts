...
export class LikelistComponent implements OnInit {
  ...
  // functions for handling pushing stories
  onAddStory(story : storyInterface) {
    this.likedStories.push(story);
    console.log(this.likedStories.length);
    // reload data?
    // this.likedStories = [...this.likedStories];
  }
  ...
  ngOnInit(): void {
    this.likeList.dataEmitter.subscribe(
      (value) => {
        console.log(value);
        this.onAddStory(value);
      }
    )
  }
