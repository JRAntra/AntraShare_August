...
export class RegisterPageComponent implements OnInit {
  ...
  showError(key: string): string {
    ...
    if (!this.registerPanel.pristine) {
      if (key === 'required') return 'This is a required field';
      else if (key === 'minlength')
        return 'Passwords have a minimum length of 8';
      else if (key === 'validusername') return 'Username already in use';
      else if (key === 'validemail') return 'Email already in use';
      else if (key === 'notcomplex')
        return 'Password must have at least 1 uppercase, 1 lowercase, 1 special character';
      else if (key === 'notsame') return 'Passwords are not matching.';
      else return '';
    } else return '';
  }
